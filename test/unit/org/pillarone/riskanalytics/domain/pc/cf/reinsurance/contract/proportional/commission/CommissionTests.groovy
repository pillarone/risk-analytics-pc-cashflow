...
import org.pillarone.riskanalytics.domain.pc.cf.reinsurance.contract.proportional.commission.applicable.ApplicableStrategyType
...
class CommissionTests extends GroovyTestCase {
    ...
    static Commission getSlidingCommission(Map<Double, Double> bands = [:]) {
        Commission commission = new Commission(
            parmCommissionStrategy : SlidingCommissionStrategyTests.getSlidingCommissionStrategy(bands),
            parmApplicableStrategy : ApplicableStrategyType.getStrategy(ApplicableStrategyType.ALL, [:])
        )
        commission.setSimulationScope getTestSimulationScope(2010)
        return commission
    }

    static CededUnderwritingInfoPacket getUnderwritingInfo(double premium, double priorCommission=0) {
        CededUnderwritingInfoPacket parent = new CededUnderwritingInfoPacket(premiumWritten: premium,
                premiumPaid: premium, commission: priorCommission)
        CededUnderwritingInfoPacket child = (CededUnderwritingInfoPacket) parent.clone()
        child.original = parent
        return child
    }

    static CededUnderwritingInfoPacket getUnderwritingInfoFromOrigin(double premiumWritten, double priorCommission=0, Component origin=null) {
        CededUnderwritingInfoPacket parent = new CededUnderwritingInfoPacket(premiumWritten: premiumWritten, commission: priorCommission,
    ...
    static CededUnderwritingInfoPacket getUnderwritingInfoFromContract(double premium, double priorCommission=0, IReinsuranceContractMarker contract) {
        CededUnderwritingInfoPacket parent = new CededUnderwritingInfoPacket(premiumWritten: premium, premiumPaid: premium,
                commission: priorCommission, reinsuranceContract: contract)
        CededUnderwritingInfoPacket child = (CededUnderwritingInfoPacket) parent.clone()
        child.original = parent
        return child
    }

    static UnderwritingInfoPacket getUnderwritingInfoFromSelf(Map parameters) {
        UnderwritingInfoPacket self = new UnderwritingInfoPacket()
    ...
    void testDefaultStrategy() {
        ...
        CededUnderwritingInfoPacket underwritingInfo200 = new CededUnderwritingInfoPacket(premiumWritten: 200, commission: 50)
        CededUnderwritingInfoPacket underwritingInfo100 = new CededUnderwritingInfoPacket(premiumWritten: 100, commission: 5)
    ...
    void testSlidingCommission() {
        ...
        ClaimCashflowPacket claim01 = ClaimCashflowPacketTests.getClaimCashflowPacket(0, 1d, null);
        ClaimCashflowPacket claim05 = ClaimCashflowPacketTests.getClaimCashflowPacket(0, 5d, null);
        ClaimCashflowPacket claim20 = ClaimCashflowPacketTests.getClaimCashflowPacket(0, 20d, null);
        ...
        assertEquals 'underwritingInfo050 (1)', 50 * 0.2, commission.outUnderwritingInfoModified[0].commission, EPSILON

        commission = getSlidingCommission()
        commission.inUnderwritingInfo << getUnderwritingInfo(-50d, 10d)
        commission.inClaims << claim01
        commission.doCalculation()
        assertEquals 'underwritingInfo100', (10 + 50 * 0.2), commission.outUnderwritingInfoModified[0].commission, EPSILON

        commission = getSlidingCommission()
        commission.inUnderwritingInfo << getUnderwritingInfo(-50d, -0d)
        commission.inClaims << claim05
        commission.doCalculation()
        assertEquals 'underwritingInfo050 (2)', 50 * 0.1, commission.outUnderwritingInfoModified[0].commission, EPSILON

        commission = getSlidingCommission()
        commission.inUnderwritingInfo << getUnderwritingInfo(-50d, -0d)
        commission.inClaims << claim20
        commission.doCalculation()
        assertEquals 'underwritingInfo050 (3)', 50 * 0.05, commission.outUnderwritingInfoModified[0].commission, EPSILON           

        commission = getSlidingCommission()
        commission.inUnderwritingInfo << getUnderwritingInfo(-50d, -0d)
        commission.inClaims << claim01 << claim05 << claim20
        commission.doCalculation()
        assertEquals 'underwritingInfo050 (4)', -0.0, commission.outUnderwritingInfoModified[0].commission, EPSILON

        commission = getSlidingCommission()
        commission.inUnderwritingInfo << getUnderwritingInfo(-50d, -0d) << getUnderwritingInfo(-60d, -0d)
        commission.inClaims << claim05 << claim20
        commission.doCalculation()
        assertEquals 'underwritingInfo050 uw1 (5)', 50 * 0.05, commission.outUnderwritingInfoModified[0].commission, EPSILON
        assertEquals 'underwritingInfo050 uw2 (5)', 60 * 0.05, commission.outUnderwritingInfoModified[1].commission, EPSILON

        commission = getSlidingCommission()
        commission.inUnderwritingInfo << getUnderwritingInfo(-50d, -0d)
        commission.inClaims
        commission.doCalculation()
        assertEquals 'underwritingInfo050 (6)', 50 * 0.2, commission.outUnderwritingInfoModified[0].commission, EPSILON
    }
    ...
    void testProfitCommission() {
        ...
        commission.setSimulationScope getTestSimulationScope(2011)

        commission.inClaims << ClaimCashflowPacketTests.getClaimCashflowPacket(0, 50, null)

        commission.inUnderwritingInfo << new CededUnderwritingInfoPacket(premiumWritten: -100, premiumPaid: -100d, commission: 0)
    ...
    void testFixedCommissionFilteringByContract() {
        ...
        CededUnderwritingInfoPacket underwritingInfo100 = getUnderwritingInfoFromContract(-100, 5, contract1)
        CededUnderwritingInfoPacket underwritingInfo200 = getUnderwritingInfoFromContract(-200, 50, contract2)
    ...
    void testProfitCommissionFilteringByContract() {
        ...
        CededUnderwritingInfoPacket underwritingInfo1 = getUnderwritingInfoFromContract(-60, 1, contract1)
        CededUnderwritingInfoPacket underwritingInfo2 = getUnderwritingInfoFromContract(-40, 2, contract2)
        CededUnderwritingInfoPacket underwritingInfo3 = getUnderwritingInfoFromContract(-20, 4, contract0)
        ...
        commission.inClaims << ClaimCashflowPacketTests.getClaimCashflowPacket(0, 10, contract0)
        commission.inClaims << ClaimCashflowPacketTests.getClaimCashflowPacket(0, 20, contract1)
        commission.inClaims << ClaimCashflowPacketTests.getClaimCashflowPacket(0, 30, contract2)
        // the claims of size 20 & 30 belong to "applicable" contracts and therefore affect the profit commission

        commission.doCalculation()

        assertEquals '# outUnderwritingInfoModified packets', 2, commission.outUnderwritingInfoModified.size()
        assertEquals 'underwritingInfo1', 1.18, commission.outUnderwritingInfoModified[0].commission, EPSILON
    ...
    void testProfitCommissionFilteringByContract2() {
        ...
        CededUnderwritingInfoPacket underwritingInfo1 = getUnderwritingInfoFromContract(-10, 1, contract1)
        CededUnderwritingInfoPacket underwritingInfo2 = getUnderwritingInfoFromContract(-20, 2, contract1)
        CededUnderwritingInfoPacket underwritingInfo3 = getUnderwritingInfoFromContract(-30, 3, contract1)
        CededUnderwritingInfoPacket underwritingInfo4 = getUnderwritingInfoFromContract(-40, 4, contract2)
        CededUnderwritingInfoPacket underwritingInfo5 = getUnderwritingInfoFromContract(-50, 5, contract2)
        CededUnderwritingInfoPacket underwritingInfo6 = getUnderwritingInfoFromContract(-60, 6, contract0)
        commission.inUnderwritingInfo << underwritingInfo1 << underwritingInfo2 << underwritingInfo3
        ...
        commission.inClaims << ClaimCashflowPacketTests.getClaimCashflowPacket(0, 10, contract1)
        commission.inClaims << ClaimCashflowPacketTests.getClaimCashflowPacket(0, 10, contract2)
        commission.inClaims << ClaimCashflowPacketTests.getClaimCashflowPacket(0, 10, contract2)
        commission.inClaims << ClaimCashflowPacketTests.getClaimCashflowPacket(0, 40, contract0)
        commission.inClaims << ClaimCashflowPacketTests.getClaimCashflowPacket(0, 50, contract0)
