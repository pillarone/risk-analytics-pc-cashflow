...
public class SingleActualClaimsStrategy extends AbstractParameterObject implements ISingleActualClaimsStrategy {
    ...
    private Map<Integer, SingleHistoricClaim> historicClaimsPerContractPeriod;
    ...
    public void lazyInitHistoricClaimsPerContractPeriod(IPeriodCounter periodCounter, DateTime updateDate, PayoutPatternBase payoutPatternBase) {
        ...
        if (historicClaimsPerContractPeriod == null) {
            historicClaimsPerContractPeriod = new HashMap<Integer, SingleHistoricClaim>();
            for (int row = history.getTitleRowCount(); row < history.getRowCount(); row++) {
                ...
                SingleHistoricClaim claim = historicClaimsPerContractPeriod.get(contractPeriod);
                if (claim == null) {
//                    claim = new SingleHistoricClaim(contractPeriod, periodCounter, payoutPatternBase);
...
//                claim.add(reportedDate, cumulativeReported, cumulativePaid);
    ...
    public void checkClaimRootOccurenceAgainstFirstActualPaid(List<ClaimRoot> baseClaims, int contractPeriod, IPeriodCounter periodCounter, DateTime updateDate, PayoutPatternBase base) {
        ...
        SingleHistoricClaim SingleHistoricClaim = historicClaimsPerContractPeriod.get(contractPeriod);
...
//        if (SingleHistoricClaim.firstActualPaidDateOrNull() != null && claimRoot.getExposureStartDate().isAfter(SingleHistoricClaim.firstActualPaidDateOrNull())) {
//            ClaimRoot claimRoot1 = new ClaimRoot(claimRoot.getUltimate(), claimRoot.getClaimType(), SingleHistoricClaim.firstActualPaidDateOrNull(), claimRoot.getOccurrenceDate());
            baseClaims.clear();
...
//        }
     ...
     * there exists a SingleHistoricClaim for contractPeriod.
     *
...
//        If the update date is the start of the first period this is an inception model, simply proceed without updating effects.
        ...
        SingleHistoricClaim historicClaim = historicClaimsPerContractPeriod.get(contractPeriod);
...
//        If there are no claim updates, we rescale the pattern to avoid stochastic payments before update date.
    ...
    public SingleHistoricClaim historicClaims(int period, IPeriodCounter periodCounter, DateTime updateDate, PayoutPatternBase base) {
        lazyInitHistoricClaimsPerContractPeriod(periodCounter, updateDate, base);
        SingleHistoricClaim historicClaim = historicClaimsPerContractPeriod.get(period);
        if (historicClaim == null) {
//            historicClaim = new SingleHistoricClaim(period, periodCounter, PayoutPatternBase.CLAIM_OCCURANCE_DATE);
        }
